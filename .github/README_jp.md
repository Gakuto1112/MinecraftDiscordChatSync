...
## 言語データの生成について
`/MinecraftDiscordChatSync/src/locales`にシステムで使用する言語データが保存されています。初期状態では英語と日本語のみが用意されていますが、ゲームが対応している言語であればそこからデータを抽出して言語データを作成することが出来ます。

言語データを作成するには**ゲームをインストールして一度起動する**必要があります。

1. ターミナルを開きます。
2. カレントディレクトリを`../MinecraftDiscordChatSync/src/locales/locale_generator`にします。
3. モジュールをインストールしていない場合は`npm install`と入力し、必要なモジュールをインストールします。
4. `ts-node LocaleDataGenerator.ts`と入力し、システムを起動します。
